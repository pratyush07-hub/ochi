...
function Navbar() {
  ...
  return (
    <div className='fixed z-[999] w-full px-4 md:px-10 lg:px-20 py-4 md:py-8 flex justify-between items-center bg-white/80 backdrop-blur-sm'>
        <div className="logo">
        ...
        {/* Mobile Menu Button */}
        <button 
            className="lg:hidden flex flex-col gap-1.5 p-2 z-50 mr-4"
            onClick={() => setIsOpen(!isOpen)}
            aria-label="Toggle menu"
        >
            <span className={`block w-8 h-0.5 bg-black transition-all duration-300 ${isOpen ? 'rotate-45 translate-y-2' : ''}`}></span>
        ...
        <div className="hidden lg:flex gap-10">
            ...
            <a key={index} className={`text-font-["Neue Montreal"] lg font-regular cursor-pointer ${index === 4 && "ml-32"}`}>{item}</a>
            ))}
        </div>

        {/* Mobile Navigation */}
        <div className={`fixed top-0 right-0 h-full w-64 bg-white shadow-lg transform transition-transform duration-300 ease-in-out lg:hidden ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
            <div className="flex flex-col p-8 mt-16 gap-6 bg-white">
                {["Services","Our work","About us","Insights","Contact us"].map((item,index) =>(
                    <a key={index} className="text-font-['Neue Montreal'] lg font-regular">{item}</a>
                ))}
            </div>
        </div>
    </div>
